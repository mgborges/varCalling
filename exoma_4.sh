...
BATCH=exoma_4
THREADS=5
DATAPATH=/home/bioinf/exoma/exoma_4/raw
...
function bwa_align_md
{
    ...
    bamout="${4}/${sn}.$lane.$fcln.bam"
    samout="${4}/${sn}.$lane.$fcln.sam"
...
function run_gatk_MERGED {
    ...
    rm ${INPUTBAM}
    ${PICARD} BuildBamIndex INPUT=${REALNBAM}
    ${GATK} -T HaplotypeCaller -R ${REF} -I ${REALNBAM} -o ${VCF} -ip 200 -ERC GVCF --variant_index_type LINEAR --variant_index_parameter 128000 -maxAltAlleles 20
    gzip ${VCF}
}
...
files=`ls $sample*recal.bam | awk '{print "INPUT=" $1}' | uniq`
...
function VEP
{
	...
	perl $VEP -i $file -o results/$name.VEP.vcf --cache  --pubmed --sift b --polyphen b --ccds --uniprot --symbol --numbers --domains --regulatory --canonical --protein --biotype --gene_phenotype --gmaf --variant_class --vcf --port 3337 --force_overwrite --merged --offline --assembly GRCh37 --fork $2
}
...
FQ1=$(ls ${DATAPATH}/$sampletoredo*R1*gz)
FQ2=$(ls ${DATAPATH}/$sampletoredo*R2*gz)

#parallel --xapply -j ${THREADS} run_fastqc ::: ${FQ1} ::: ${FQ2} ::: qc

parallel --xapply -j ${THREADS} bwa_align_md ::: ${FQ1} ::: ${FQ2} ::: 2 ::: bams

parallel -j ${THREADS} run_gatk ::: $(ls bams/$sampletoredo*bam)
parallel -j ${THREADS} picard_hsmetrics ::: $(ls bams/$sampletoredo*bam) ::: qc/aln

samples=`ls bams/$sampletoredo*recal.bam | cut -f1 -d. | awk -F "bams/" '{print $2}' | sort | uniq`

parallel -j ${THREADS} mergeBAM ::: ${samples}

parallel -j ${THREADS} run_gatk_MERGED ::: $(ls bams/$sampletoredo*.merged.bam)

parallel -j ${THREADS} picard_hsmetrics ::: $(ls bams/$sampletoredo*.merged.bam.realn.bam) ::: qc/aln

myfiles=`ls results/*.g.vcf`
gVCFlist=`echo $myfiles | sed 's/ / -V /g' | sed 's/^/-V /g'`

$GATK -T GenotypeGVCFs -R $REF -o results/$BATCH.vcf $gVCFlist --max_alternate_alleles 20

gzip results/*g.vcf

parallel -j 1 VEP ::: $(ls results/*.vcf) ::: ${THREADS}
