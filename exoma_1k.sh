BATCH=1kg
THREADS=14
DATAPATH=/home/murilo/realn_BIPMED/1kg/raw
...
function haplotypecaller {
    ...
    VCF="/iscsi/murilo/realignment_article/1k_exom/$folder.vcf"
    java -jar /opt/GenomeAnalysisTK.jar -T HaplotypeCaller -R ${REF} $INPUT -o ${VCF} -rf BadCigar --max_alternate_alleles 100
}
...
FQ1=$(ls ${DATAPATH}/*GBR*R1*REPAIR*gz)
FQ2=$(ls ${DATAPATH}/*GBR*R2*REPAIR*gz)
...
do 
R2=`echo $R1 | sed 's/R1/R2/g'`
bwa_align_md $R1 $R2 ${THREADS} BAMS_NO_REALN
done
...
cd /iscsi/murilo/realignment_article/realn_BIPMED/1kg

haplotypecaller BAMS_1K_CLM &
haplotypecaller BAMS_1K_GBR &
haplotypecaller BAMS_1K_PEL &

haplotypecaller BAMS_NO_REALN_CLM &
haplotypecaller BAMS_NO_REALN_GBR &
haplotypecaller BAMS_NO_REALN_PEL &

haplotypecaller BAMS_BIPMED_CLM &
haplotypecaller BAMS_BIPMED_GBR &
haplotypecaller BAMS_BIPMED_PEL &


cd /iscsi/murilo/realignment_article/1k_exom/NOFILTER

for VCF in `ls *vcf`
do
java -jar /opt/snpEff/SnpSift.jar filter "(DP>10)" -a "(QUAL>30)" $VCF > ../$VCF.DP.vcf &&
vcftools --vcf ../$VCF.DP.vcf --recode --out ../$VCF.DP.GENE &&
mv ../$VCF.DP.GENE.recode.vcf ../$VCF.DP.QUAL.vcf &
done


cd /iscsi/murilo/realignment_article/1k_exom/FILTERED
